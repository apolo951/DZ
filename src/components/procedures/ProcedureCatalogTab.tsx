...
import { ProcedureDocumentImportModal } from './ProcedureDocumentImportModal';
import { ProcedureReportIssueModal } from './ProcedureReportIssueModal';
...
export function ProcedureCatalogTab({ onAddProcedure, onOpenApprovalQueue }: ProcedureCatalogTabProps) {
  ...
  const [isProceduresImportOpen, setIsProceduresImportOpen] = useState(false);
  const [isProceduresReportOpen, setIsProceduresReportOpen] = useState(false);
  ...
  return (
    <div className="space-y-6">
            ...
            <Card key={procedure.id} className="hover:shadow-md transition-shadow">
              <CardContent className="p-6">
                ...
                <div className="flex gap-2 mt-4 justify-end">
                  ...
                  <Button variant="outline" size="sm" onClick={() => handleDownloadProcedure(procedure)}>
                    <Download className="w-4 h-4 mr-1" />
                    Télécharger
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => handleShareProcedure(procedure)}>
      ...
      <ProcedureDocumentImportModal
        isOpen={isProceduresImportOpen}
        onClose={() => setIsProceduresImportOpen(false)}
      />

      <ProcedureReportIssueModal
        isOpen={isProceduresReportOpen}
        onClose={() => setIsProceduresReportOpen(false)}
      />
      {isConsultationModalOpen && selectedProcedure && (
        <Dialog open={isConsultationModalOpen} onOpenChange={setIsConsultationModalOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>{selectedProcedure.title || 'Consultation de procédure'}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div><span className="font-semibold text-gray-700">Type :</span> {selectedProcedure.type}</div>
              <div><span className="font-semibold text-gray-700">Statut :</span> {selectedProcedure.status}</div>
              <div><span className="font-semibold text-gray-700">Catégorie :</span> {selectedProcedure.category}</div>
              <div><span className="font-semibold text-gray-700">Institution :</span> {selectedProcedure.organization}</div>
              <div><span className="font-semibold text-gray-700">Durée :</span> {selectedProcedure.duration}</div>
              <div><span className="font-semibold text-gray-700">Complexité :</span> {selectedProcedure.complexity}</div>
              <div><span className="font-semibold text-gray-700">Dernière mise à jour :</span> {selectedProcedure.lastUpdate}</div>
              <div><span className="font-semibold text-gray-700">Description :</span>
                <div className="text-gray-800 mt-1">{selectedProcedure.description}</div>
              </div>
              <div>
                <span className="font-semibold text-gray-700">Documents requis :</span>
                <ul className="list-disc ml-6 text-gray-800">
                  {selectedProcedure.requiredDocuments?.map((doc: string, idx: number) => (
                    <li key={idx}>{doc}</li>
                  ))}
                </ul>
              </div>
              <div>
                <span className="font-semibold text-gray-700">Étapes :</span>
                <ol className="list-decimal ml-6 text-gray-800">
                  {selectedProcedure.steps?.map((step: string, idx: number) => (
                    <li key={idx}>{step}</li>
                  ))}
                </ol>
              </div>
            ...
            <div className="flex justify-end mt-6">
              <Button variant="outline" onClick={() => setIsConsultationModalOpen(false)}>Fermer</Button>
            </div>
          </DialogContent>
        </Dialog>
      )}
      {isShareModalOpen && selectedProcedure && (
        <Dialog open={isShareModalOpen} onOpenChange={setIsShareModalOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Partager cette procédure</DialogTitle>
            </DialogHeader>
            <div className="space-y-2">
              <div className="text-gray-700">Lien de partage :</div>
              <div className="flex items-center gap-2">
                <Input readOnly value={`${window.location.origin}/procedure/${selectedProcedure.id}`} className="flex-1" />
                <Button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText(`${window.location.origin}/procedure/${selectedProcedure.id}`);
                  }}
                  size="sm"
                  variant="outline"
                >

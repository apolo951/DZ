...
export function SecuritySection({ language = "fr" }: SecuritySectionProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (tabParam && ['status','policies','threats','audit','governance','rgpd','advanced'].includes(tabParam)) {
        setActiveTab(tabParam);
      }
    } catch (error) {
      console.warn('Failed to parse URL parameters for security tab:', error);
    }
  }, []);
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="status" className="space-y-4">
          {/* Bloc Runtime config visible aussi dans Statut Sécurité */}

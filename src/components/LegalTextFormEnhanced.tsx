...
import { extractLegalTextData } from '@/utils/ocrFormFiller';
...
function LegalTextFormEnhanced({ 
  ...
  }, [customForms.length, uniqueLegalTextForms.length, availableTypes.length, availableCategories.length, availableTypes, availableCategories]);

  const handleOCRFormDataExtracted = useCallback((data: { documentType: 'legal', formData: Record<string, any> }) => {
    if (process.env.NODE_ENV === 'development') {
  ...
  }, [uniqueLegalTextForms, selectedForm, setInputMethod, toast, mapAlgerianOCRDataToForm, mapOCRDataToForm, selectedTextType, validateAlgerianDocument]);
  ...
  }, [ocrData, handleOCRFormDataExtracted]);

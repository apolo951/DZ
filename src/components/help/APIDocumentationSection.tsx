...
curl -X POST "/api/auth/login" \\
  ...
  const handleTestApi = () => {
    // Ouvre via Provider avec le même composant (UI identique)
    openModal({
      id: `api-test-${Date.now()}`,
      type: 'display',
      title: "Test de l'API",
      content: (
        <ApiTestingModal isOpen={true} onClose={() => {}} />
      ),
      size: 'xl'
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Modales locales conservées pour compatibilité, mais non utilisées quand ouvert via Provider */}
    </div>
  );
}

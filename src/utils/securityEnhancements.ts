  ...
  private scanResources() {
    try {
      ...
      for (const entry of entries) {
        // Allow localhost and development resources
        if (entry.name.includes('localhost') || entry.name.includes('127.0.0.1') || entry.name.includes('ws://localhost') || entry.name.includes('ws://127.0.0.1')) {
          continue;
        }

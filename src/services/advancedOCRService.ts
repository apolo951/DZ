...
async function loadPDFJS() {
  ...
  try {
    ...
    // @ts-ignore
    pdfjsLib.GlobalWorkerOptions.workerSrc = /* @vite-ignore */ new URL('pdfjs-dist/build/pdf.worker.min.js', import.meta.url).toString();
  }
  __pdfjsLib = pdfjsLib;
  return __pdfjsLib;
}
...
/**
 * Étape 1: Extraction du texte et des tables avec PDF.js (RÉELLE)
 */
export class TextTableExtractor {
  async extractFromPDF(file: File): Promise<{ text: string; tables: ExtractedTable[]; zones: ExtractedTextZone[] }> {
    logger.info('OCR', '🔄 [Étape 1] Extraction RÉELLE du texte et des tables...');
    
    try {
      // Extraction du texte principal avec PDF.js (RÉELLE)
      const { text } = await extractTextFromPDF(file);
      
      // Détection RÉELLE des zones et tables selon l'algorithme annexe
      const zones = await this.detectTextZones(text);
      const tables = await this.detectTables(text);
      
      logger.info('OCR', '✅ [Étape 1] Extraction RÉELLE terminée', {
      ...
      logger.error('OCR', '❌ [Étape 1] Erreur d\'extraction RÉELLE:', error);
      throw new Error(`Erreur d'extraction RÉELLE: ${error.message}`);
    }
  }
  ...
  private async detectTables(text: string): Promise<ExtractedTable[]> {
    // Détection RÉELLE de tables basée sur des patterns réguliers
    const tables: ExtractedTable[] = [];
    
    // Recherche de structures tabulaires RÉELLES
    const tablePattern = /(\|[^|\n]+)+\|/g;
    let match;
    ...
    // Détection de tables basées sur l'alignement (espaces multiples)
    const lines = text.split('\n');
    const tableSections = this.findTableSections(lines);
    
    for (const section of tableSections) {
      const table = this.parseTableFromLines(section);
  ...
  private findTableSections(lines: string[]): string[][] {
    const sections: string[][] = [];
    let currentSection: string[] = [];
    
    for (const line of lines) {
      // Détecter les lignes qui ressemblent à des tables (espaces multiples)
      if (line.match(/\s{3,}/) && line.length > 30) {
  ...
  private parseTableFromLines(lines: string[]): ExtractedTable {
    const cells: string[][] = [];
    
    for (const line of lines) {
      const row = line.split(/\s{3,}/).map(cell => cell.trim());
      if (row.length > 1) {
        cells.push(row);
      }
    }
    ...
    return {
      cells,
      position: { x: 0, y: 0, width: 100, height: cells.length * 20 },
      confidence: 0.80
    };
  }
}
...
/**
 * Étape 3: Traitement NLP et extraction d'entités (RÉELLE avec patterns algériens)
 */
export class NLPProcessor {
  async extractEntities(text: string, structuredData: StructuredPublication): Promise<LegalEntity[]> {
    logger.info('OCR', '🔄 [Étape 3] Traitement NLP RÉEL et extraction d\'entités...');
    
    const entities: LegalEntity[] = [];
    
    // Extraction RÉELLE d'entités avec patterns algériens
    ...
    logger.info('OCR', '✅ [Étape 3] Extraction RÉELLE d\'entités terminée', { entitiesCount: entities.length });
 ...
 * Utilise PDF.js (via PyScript/Emscripten), spaCy local, et Tesseract.js
 */
export class LocalAlgerianOCRService {
  ...
  /**
   * Extraction OCR complète avec PDF.js local + Tesseract.js
   */
  async extractWithLocalTools(file: File): Promise<{
    ...
    try {
      // Étape 1: Extraction avec PDF.js local
      const { pdfText, pages } = await this.extractWithLocalPDFJS(file);
  ...
  /**
   * Extraction PDF.js locale (via WebAssembly si disponible, sinon PDF.js)
   */
  private async extractWithLocalPDFJS(file: File): Promise<{ pdfText: string; pages: ImageData[] }> {
    logger.info('PDF.js Local', '📄 [PDF.js Local] Extraction du PDF...');
    
    try {
      // Tentative d'utilisation de PDF.js via WebAssembly (si disponible)
      if (typeof window !== 'undefined' && (window as any).PDF.js) {
        // Méthode supprimée, utilisation directe de PDF.js
      }
      ...
      logger.warn('PDF.js', '⚠️ [PDF.js] Fallback vers PDF.js:', error);
      return await this.extractWithLocalPDFJS(file);
    }
  }
...
const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
  ...
  private pixmapToImageData(pixmap: any): ImageData {
    // Conversion PDF.js Pixmap vers ImageData

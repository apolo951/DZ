...
// Chargement centralisé de PDF.js
import { loadPDFJS } from '@/utils/pdfWorkerSetup';

let __pdfjsLib: any = null;
export async function loadPDFJSCached() {
  if (__pdfjsLib) return __pdfjsLib;
  __pdfjsLib = await loadPDFJS();
  return __pdfjsLib;
}
...
export class AlgerianLegalOCRService {
  private tesseractWorker: Record<string, unknown> | null = null;
  private initAttempted = false;
  private initFailed = false;

  constructor() {
    // Lazy init pour éviter les erreurs CSP en iframe; initialisation au premier usage
  }
  ...
  private async initializeTesseract() {
    ...
    try {
      // Configuration Tesseract.js avec fallback pour éviter les erreurs CDN
      console.log('🔄 [TESSERACT] Initialisation du worker OCR...');
      ...
      console.warn("Tesseract non initialisé (probable CSP en iframe):", (error as any)?.message || error);
  ...
  async extractFromPDF(file: File): Promise<ExtractedData> {
    ...
    try {
      ...
      for (let pageIndex = 0; pageIndex < pages.length; pageIndex++) {
        ...
        // Étape 4.1: Pré-traitement amélioré (deskew + binarisation + débruitage)
        const enhancedImage = await this.preprocessImageEnhanced(cleanedImage);
        ...
        const tables = await this.detectTables(lines, enhancedImage);
        
        // Étape 7: Extraire les rectangles représentant les zones de textes et les tables
        const rectangles = await this.extractRectangles(lines, tables, enhancedImage);
        
        // Étape 8: Pour chaque rectangle
        for (const rectangle of rectangles) {
          if (rectangle.type === 'text') {
            // Étape 10: Extraire le texte contenant dans le rectangle
            const textContentRaw = await this.extractTextFromRectangle(enhancedImage, rectangle);
            const textContent = this.postProcessOCRErrors(textContentRaw);
            allText += textContent + '\n';
          } else {
...
const pdfjsLib = await loadPDFJSCached();
const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
  ...
  private async extractTextFromRectangle(imageData: ImageData, rectangle: Rectangle): Promise<string> {
    ...
    try {
      ...
      return this.postProcessOCRErrors(text.trim());
  ...
  /**
   * Pré-traitement amélioré : deskew + binarisation adaptative + débruitage
   */
  private async preprocessImageEnhanced(imageData: ImageData): Promise<ImageData> {
    try {
      const cvAny: any = (window as any).cv;
      if (!cvAny || typeof cvAny.Mat !== 'function') return imageData;

      const mat = cvAny.matFromImageData(imageData);
      const gray = new cvAny.Mat();
      cvAny.cvtColor(mat, gray, cvAny.COLOR_RGBA2GRAY);

      const bin = new cvAny.Mat();
      cvAny.adaptiveThreshold(gray, bin, 255, cvAny.ADAPTIVE_THRESH_GAUSSIAN_C, cvAny.THRESH_BINARY, 35, 15);

      const kernel = cvAny.getStructuringElement(cvAny.MORPH_RECT, new cvAny.Size(3, 3));
      const denoised = new cvAny.Mat();
      cvAny.morphologyEx(bin, denoised, cvAny.MORPH_OPEN, kernel);

      const out = new ImageData(imageData.width, imageData.height);
      const outRGBA = new Uint8ClampedArray(imageData.width * imageData.height * 4);
      for (let i = 0, j = 0; i < denoised.data.length; i++, j += 4) {
        const v = denoised.data[i];
        outRGBA[j] = v; outRGBA[j + 1] = v; outRGBA[j + 2] = v; outRGBA[j + 3] = 255;
      }
      out.data.set(outRGBA);

      mat.delete(); gray.delete(); bin.delete(); denoised.delete(); kernel.delete?.();
      return out;
    } catch {
      return imageData;
    }
  ...
  /**
   * Post-traitement OCR : corrections FR/AR de base (accents/diacritiques, confusions courantes)
   */
  private postProcessOCRErrors(text: string): string {
    if (!text) return text;
    let t = text;
    // Corrections françaises simples
    t = t.replace(/\b1oi\b/gi, 'loi');
    t = t.replace(/décret\s+executif/gi, 'décret exécutif');
...
// Point d'entrée canonique simple
export async function extractText(input: File | ImageData): Promise<{ text: string; confidence: number }> {
  try {
    if (input instanceof File) {
      if (input.type === 'application/pdf') {
        const result = await algerianOCRService.extractFromPDF(input);
        return { text: result.text.trim(), confidence: Math.round(result.confidence) };
      }
      // Image file: OCR direct via Tesseract avec configuration simple
      console.log('🔄 [TESSERACT] Traitement image direct...');
    ...
    console.log('🔄 [TESSERACT] Traitement ImageData...');
    const worker = await createWorker(['fra', 'ara'], 1, { 
      gzip: false,
      cacheMethod: 'none',
      logger: m => console.log('📄 [TESSERACT]', m)

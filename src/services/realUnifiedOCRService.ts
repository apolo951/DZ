...
class RealUnifiedOCRService {
  ...
  async extractText(input: File | HTMLCanvasElement | ImageData | Blob): Promise<RealOCRResult> {
    ...
    try {
      ...
      console.log(`🔍 [UNIFIÉ-DÉTECTION] Arabe: ${arabicMatches.length}, Français: ${frenchMatches.length}, Ratio arabe: ${Math.round(arabicRatio * 100)}%, Langue: ${language}`);

      console.log(`✅ [RÉEL] OCR terminé: ${extractedText.length} caractères, confiance: ${(confidence * 100).toFixed(1)}%`);

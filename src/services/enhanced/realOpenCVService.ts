...
class RealOpenCVService {
  ...
  private async extractTableFromRegion(
    ...
    try {
      ...
      const cells = this.extractCells(roi, rows, cols, region);
  ...
  ): TableExtraction['tables'][0]['cells'] {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        cells.push({
          row,
          col,
          text: `Cell(${row},${col})`, // Dans une implémentation réelle, on ferait de l'OCR ici
          bounds: {

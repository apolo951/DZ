...
class FinalRealOCRService {
  ...
  private async performInitialization(): Promise<void> {
    try {
      ...
      this.worker = await createWorker(['ara', 'fra'], 1, {
        logger: (m: any) => console.log('🔍 [TESSERACT-FINAL-ARABE]', m),
        errorHandler: (err: any) => console.warn('⚠️ Tesseract warning (final):', err)
      });
      ...
      extractionStatus.logSimulationError('OCR', 'tesseract-init', `Erreur: ${error.message}`);
      throw new Error(`Échec initialisation OCR: ${error.message}`);
    }
  }
  ...
  async extractFromImage(file: File): Promise<FinalRealOCRResult> {
    ...
    try {
      ...
      console.log(`🔍 [FINAL-DÉTECTION] Arabe: ${arabicMatches.length}, Français: ${frenchMatches.length}, Ratio arabe: ${Math.round(arabicRatio * 100)}%, Langue: ${language}`);
      ...
      extractionStatus.logRealExtraction('OCR', file.name, true, 
        `${extractedText.length} caractères, confiance: ${(confidence * 100).toFixed(1)}%`);

      console.log(`✅ [FINAL-RÉEL] Image traitée: ${extractedText.length} caractères, confiance: ${(confidence * 100).toFixed(1)}%`);
      
      return finalResult;
      
    } catch (error) {
      console.error('❌ [FINAL-RÉEL] Erreur extraction image:', error);
      extractionStatus.logRealExtraction('OCR', file.name, false, `Erreur: ${error.message}`);
      throw new Error(`Extraction image échouée: ${error.message}`);
    }
  }
  ...
  async extractFromPDF(pdfFile: File): Promise<FinalRealOCRResult> {
    ...
    try {
      ...
      extractionStatus.logRealExtraction('PDF', pdfFile.name, false, `Erreur: ${error.message}`);

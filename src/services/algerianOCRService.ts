...
class AlgerianOCRService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const frenchWorker = await createWorker('fra', 1, {
        logger: m => console.log('[Tesseract FR]', m)
      });
      ...
      const arabicWorker = await createWorker('ara', 1, {
        logger: m => console.log('[Tesseract AR]', m)
      });
      await arabicWorker.loadLanguage('ara');
      await arabicWorker.initialize('ara');
      await arabicWorker.setParameters({
        tessedit_char_whitelist: 'أبتثجحخدذرزسشصضطظعغفقكلمنهويىءآإؤئ0123456789°-/.,;:()[]{}«»"\'ـ ',
        tessedit_pageseg_mode: '6', // Mode bloc uniforme pour l'arabe
        tessedit_ocr_engine_mode: '2', // LSTM uniquement
        preserve_interword_spaces: '1',
